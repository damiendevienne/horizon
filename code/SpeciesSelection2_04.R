...
# main parameters
{
  setwd("/Users/Sylvain/Documents/Recherche/Horizon/specimen_selection/Version_02/code")
  nb_run <- 2 # number of samples to be obtained
  maxSP <-100 #max number of species to select (stoping criteria) <- 70 # number of species to be sampled
  # run_id <- args[1] # id of this run; UNcomment if using Rscript call
  run_id <- "batch_025" # id of this run; COMMENT if using Rscript call
  # triplets computing parameters
  S_lien=15
  S_nl=30
  D_seuil_min=0.1
  max_tach_prop <- 1/3 # the maximum proportion of tachinid (above this, tach triplets are removed at each round)
  max_par_prop <- 0.55 # the maximum proportion of parasitoid species (above this, P duplets are removed at each round)
  min_par_prop <- 1- max_par_prop # the minum proportion of parasitoid species (above this, H duplets are removed at each round)
  n_rand_init <- 10
}

### optimization parameters
{
  Freq.remove<-5 #every Freq.remove round, instead of adding best triplet, we remove the one with less impact when removed.
  Freq.reload_all_TRIPLETS<-20
  ...
  SampleSizeAvailable<-200 # number of best triplets to consider at all round
  #SAVESCORE<-array() #TO SAVE SCORES
}

### functions
{
  # A function to produce nbgroup clusters or optimally even size from a tree
  ...
  # a function to get HHP and PPH triplets
  ...
  # a function to get plant tripelts (HHH)
  ...
  # a function to get all SGP duplets: pairs of parasites that are closely related (same cluster) and specialist vs generalist
  GetSGPDuplets<-function(threshold_n_obs=10, D_seuil_min_SG=0.1) {
    
    par_spec_tab <- P
    good_par_spec_tab <- par_spec_tab[which(par_spec_tab$n_obs_with_host>=threshold_n_obs),]
    
    low_par_spec_index <- quantile(good_par_spec_tab$spec_index, 0.25)
    high_par_spec_index <- quantile(good_par_spec_tab$spec_index, 0.75)
    ...
    for (i in 1:nrow(SGPDup_tab)) {
      SGPDup_mat[as.character(SGPDup_tab[i,1]),as.character(SGPDup_tab[i,2])]<-SGPDup_tab$group[i]
      SGPDup_mat[as.character(SGPDup_tab[i,2]),as.character(SGPDup_tab[i,1])]<-SGPDup_tab$group[i]
  ...
  # A function to get a list of species from a list of triplets in string format (with & separators)
  ...
  # A (similar) function to get triplet scores from a list of triplets strings + one new potential triplet string
  ...
  # A function to get triplet scores when removing this triplet from the selected list
  ...
  # A function to convert a list of species to a list of selected triplets, and a list of availbalble triplets, given a list of total triplets.
  ...
  # A to print relevant information
  ...
  # a function to get the proportion of parasitoid species
  GetParProp<-function(selectedtriplets) {
    selectedtriplets <- str2trip(selectedtriplets)
    allmono <- unique(c(selectedtriplets[,1], selectedtriplets[,2],selectedtriplets[,3]))
    P_sps <- row.names(P)[which(is.element(row.names(P),allmono))]
    P_prop <- length(P_sps)/length(allmono)
    return(P_prop)
  }
...
### compute or read triplets and cubes
{
  ...
  print("triplets computed...")
  
  SGPDup_mat<-GetSGPDuplets(threshold_n_obs=10, D_seuil_min_SG=0.1)
  
  print("SG duplets computed...")
...
### RUN: sampling n_sp species with the greedy algorithm, nb_run times, and store results
{
  ...
  # for all runs
  {
    ...
    # declare empty tables
    sel_triplets <- data.frame(matrix(NA, nrow = 1000, ncol = nb_run))
    sel_triplets_scores <- NULL
    
    #get the list of all parasitoid species
    all_par_sps <- row.names(P)
    
    #get the list of all tachinid species
    all_tach_sps <- row.names(P)[which(P$family=="Tachinidae")]
    
  }
  
  # For each run
  for (run_i in c(1:nb_run)){
    ...
    #### INITIATION 
    {
      ...
      ##### and the best selection of triplets encounterd (highest score) is kept. 
      #####
      startscore<-0
      for (i in 1:n_rand_init) {
      ...
      # compute parasitoid proportion after random phase
      current_par_prop <- GetParProp(SelectedTrip)
    ...
    while (length(GetAllMono(SelectedTrip))<maxSP) {
      ...
      if (cpt%%Freq.remove==0) {
        ...
        cat(paste("removed one triplet","\n"))
      }
      
      else {
        
        # apply filters to refine the list of triplets from which to choose
        {
...
#          AvailableTripTab <- str2trip(AvailableTrip)
          ...
          #   par_sps_pos <- which(is.element(sps_1,all_par_sps))
          #   AvailableTrip<-AvailableTrip[-par_sps_pos]
          # }
          # 
    ...
    # put final scores + LIST OF TRIPLETS from this run in output tabS
    trip_res <- c(GetScoresDetails[,TripletOfInterest], round(geomean(GetScoresDetails[,TripletOfInterest]), digits = 2))
    sel_triplets_scores<-cbind(sel_triplets_scores,trip_res)
    
    
    sel_triplets[c(1:length(SelectedTrip)),run_i] <- SelectedTrip
    
    
  }
  
  # for all runs: write ouptut tables and print time
  {
    colnames(sel_triplets) <- c(1:nb_run)
    colnames(sel_triplets) <- c(1:nb_run)
    write.table(sel_triplets,paste("../output/sel_triplets_",run_id,".tab",sep=""), row.names=FALSE, quote=F,sep="\t")
    scores_names <- c("trip_HHP", "trip_PPH", "trip_HHH", "SGP_score", "whole_score")
    write.table(sel_triplets_scores,paste("../output/sel_triplets_scores_",run_id,".tab",sep=""), col.names=F, row.names=scores_names, quote=F,sep="\t")
    
    # print duration
    end_time <- Sys.time()
    print(paste("run duration:", end_time-start_time, sep=""))
